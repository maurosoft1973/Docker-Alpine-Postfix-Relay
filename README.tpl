...
This Docker image [(maurosoft1973/alpine-postfix-relay)](https://hub.docker.com/r/maurosoft1973/alpine-postfix-relay/) is based on the minimal [Alpine Linux](https://alpinelinux.org/) with [Postfix Version v%POSTFIX_VERSION%](https://www.postfix.net).

##### Alpine Version %ALPINE_VERSION% (Released %ALPINE_VERSION_DATE%)
##### Postfix Version %POSTFIX_VERSION% (Released %POSTFIX_VERSION_DATE%)
...
* ```:%ALPINE_VERSION%-%POSTFIX_VERSION%``` %ALPINE_VERSION%-%POSTFIX_VERSION% branch based (Automatic Architecture Selection)
* ```:%ALPINE_VERSION%-%POSTFIX_VERSION%-aarch64```   %ALPINE_VERSION% 64 bit ARM
* ```:%ALPINE_VERSION%-%POSTFIX_VERSION%-armhf```     %ALPINE_VERSION% 32 bit ARM v6
* ```:%ALPINE_VERSION%-%POSTFIX_VERSION%-armv7```     %ALPINE_VERSION% 32 bit ARM v7
* ```:%ALPINE_VERSION%-%POSTFIX_VERSION%-ppc64le```   %ALPINE_VERSION% 64 bit PowerPC
* ```:%ALPINE_VERSION%-%POSTFIX_VERSION%-x86```       %ALPINE_VERSION% 32 bit Intel/AMD
* ```:%ALPINE_VERSION%-%POSTFIX_VERSION%-x86_64```    %ALPINE_VERSION% 64 bit Intel/AMD
